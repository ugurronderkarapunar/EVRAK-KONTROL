...
Açıklama   : Personele ait nitelik belgelerinin bitiş tarihlerini izler,
             ...
             v1.4.1: localStorage reload sorunu düzeltildi.
...
# ══════════════════════════════════════════════════════════════════════════════
# localStorage KÖPRÜSÜ (Düzeltilmiş, reload yapmadan çalışır)
# ══════════════════════════════════════════════════════════════════════════════

# Session state'te unvan_map yoksa oluştur
if "unvan_map" not in st.session_state:
    st.session_state["unvan_map"] = {}

# localStorage'daki veriyi URL query param üzerinden session state'e aktar
# Bu işlem yalnızca URL'de "_u" parametresi varsa ve session state boşsa yapılır.
query_data = st.query_params.get("_u", "")
if query_data and not st.session_state["unvan_map"]:
    try:
...
# Her render'da güncel unvan_map'i localStorage'a yaz (veri kaybı olmasın)
...
# Eğer localStorage'da veri var ama URL'de "_u" yoksa, URL'ye ekleyip
# sayfayı **sadece bir kez** yenileyelim. Bu sayede sonraki yüklemelerde
# URL'den okuyup session state'i doldurabiliriz.
# Ancak bu yenileme, yalnızca sayfa ilk açıldığında ve localStorage doluysa olur.
# Kullanıcı dosya yükledikten sonra tekrar olmaz.
components.html("""
<script>
(function() {
    ...
    if (!url.searchParams.has('_u')) {
        ...
        if (stored) {
            ...
            // Yalnızca localStorage'da veri varsa ve URL'de yoksa sayfayı yenile
...
with st.sidebar:
    ...
    st.markdown(
        ...
        "v1.4.1 · Nitelik Belgesi Takip Sistemi<br>Streamlit + pandas + plotly"
...
with tab_panel:
    ...
    with st.expander(expander_baslik, expanded=(atanmamis_sayisi > 0)):
        ...
        if st.button("🗑 Tüm ünvan atamalarını sıfırla", key="reset_unvanlar"):
            st.session_state["unvan_map"] = {}
            reset_localstorage()
            # URL'deki _u parametresini temizle ve sayfayı yenile
